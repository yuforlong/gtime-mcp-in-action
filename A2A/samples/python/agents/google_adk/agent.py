...
class ReimbursementAgent(AgentWithTaskManager):
  ...
  async def stream(self, query, session_id) -> AsyncIterable[Dict[str, Any]]:
        ...
        # Here's where we need to correctly handle the async generator
        async for event in self._runner.run_async(
            ...
            if event.is_final_response():
                ...
                if (
                    ...
                    response = next((p.function_response.model_dump() for p in event.content.parts if p.function_response), None)
